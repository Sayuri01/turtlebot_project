...
void SlamGMapping::init()
{
  ...
  //*** Added by Linda ***//
  //object = 0;
  //Register the subscriber to suscribe to /object topic which will update the map
  //objectSubscriber_ = private_nh_.subscribe("/object", 1, &SlamGMapping::objectCallback, this);
  
  obstaclePosition.data.resize(3);
  // Create the publisher to publish the /obstacle topic which will return 
  // 1 if an obstacle is detected and 0 otherwise
  obstaclePositionPub_ = private_nh_.advertise<std_msgs::Float64MultiArray>("obstaclePosition", 1, true);
  //**********************//
}

//*** Added by Linda ***//
/*void SlamGMapping::objectCallback(const std_msgs::Float64& detectedObject)
{
    object = detectedObject.data;
    printf("Object: %f\n", object);
}*/
//**********************//
  ...
  for(int x=0; x < smap.getMapSizeX(); x++)
  {
    for(int y=0; y < smap.getMapSizeY(); y++)
    {
      ...
      if(occ < 0){
        ...
        //*** Added by Linda ***//
        /*obstaclePosition.data[0] = 0;
        obstaclePosition.data[1] = 0;
        obstaclePosition.data[2] = 0;
        obstaclePositionPub_.publish(obstaclePosition);*/
        //**********************//
      }
      else if(occ > occ_thresh_)
      {
        ...
        //*** Added by Linda ***//
        /*if(object != 0){
            map_.map.data[MAP_IDX(map_.map.info.width, x-1, y-1)] = object;
        }*/
        obstaclePosition.data[0] = 1;
        obstaclePosition.data[1] = x-1;
        obstaclePosition.data[2] = y-1;
        obstaclePositionPub_.publish(obstaclePosition);
        //**********************//
      }
      else{
